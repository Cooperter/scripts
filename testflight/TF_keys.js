...
const $ = new Env("获取TestFlight账户信息");
...
if (reg1.test($request.url)) {
  ...
  data.key = key
  data.sessionId = headers["x-session-id"];
  data.sessionDigest = headers["x-session-digest"];
  data.requestId = headers["x-request-id"];
  $.setjson(data, 'TEST_FLIGHT')

  const testflight = $.getjson('TEST_FLIGHT')
  if ($.lodash_get(testflight, 'requestId') !== null) {
    $.msg("TestFlight自动加入", "信息获取成功", "");
  } else {
    $.msg("TestFlight自动加入", "信息获取失败", "请添加testflight.apple.com");
  }

  console.log(`写入数据key:${$.lodash_get(testflight, 'key')}`);
  console.log(`写入数据sessionId:${$.lodash_get(testflight, 'sessionId')}`);
  console.log(`写入数据sessionDigest:${$.lodash_get(testflight, 'sessionDigest')}`);
