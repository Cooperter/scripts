...
//*********************************
// */1 * * * * * https://raw.githubusercontent.com/tanzl/scripts/main/testflight/Auto_join_TF.js, tag=TestFlight自动加入, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/testflight.png, enabled=true
//*********************************
...
async function autoPost(appId) {
  const { key, sessionId, sessionDigest, requestId } = testflight
  ...
  return new Promise(resolve => {
    $.get({ url: testurl, headers }, async (err, resp, body) => {
      ...
      if (status === 404) {
        ...
        if (jsonData.data == null) {
          ...
          console.log(appId + " " + jsonData.data.message);
        } else {
          $.post({ url: testurl + appId + "/accept", headers }, async (err, resp, body) => {
            const status = $.lodash_get(resp, 'statusCode')
            $.log(`status: ${status}`)
...
async function getQlToken() {
  ...
  const authConfig = JSON.parse(auth);
  return authConfig.token;
}

async function getQlEnvs() {
  const token = await getQlToken();
  const res = await $.http.get({
    url: 'http://127.0.0.1:5600/api/envs',
    searchParams: {
      searchValue: 'TEST_FLIGHT',
      t: Date.now(),
    },
    headers: {
...
async function updateQlEnvs(appIds) {
  let env = await getQlEnvs()
  const { id, name } = env
  testflight.appId = appIds || ''
